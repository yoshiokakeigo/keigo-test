<x-guest-layout>
    <!-- Session Status -->
    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('login') }}">
        @csrf

        <!-- Email Address -->
        <div class="form-address">
            <x-input-label class="text-white text-xl" for="email" :value="__('Email')" />
            <x-text-input id="email" class="under-line block mt-1 w-full box-none" type="email" name="email"
                :value="old('email')" required autofocus autocomplete="username" />
            <x-input-error :messages="$errors->get('email')" class="mt-2" />
        </div>

        <!-- Password -->
        <div class="mt-4 form-password">
            <x-input-label class="text-white text-xl" for="password" :value="__('Password')" />

            <x-text-input id="password" class="under-line block mt-1 w-full box-none" type="password" name="password"
                required autocomplete="current-password" />

            <x-input-error :messages="$errors->get('password')" class="mt-2" />
        </div>

        <!-- Remember Me -->
        <div class="block mt-4">
            <label for="remember_me" class="inline-flex items-center">
                <input id="remember_me" type="checkbox"
                    class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" name="remember">
                <span class="ml-2 text-sm font-semibold text-white">{{ __('Remember me') }}</span>
            </label>
        </div>

        <div class="flex items-center mt-4">
            @if (Route::has('password.request'))
                <a class="underline text-sm font-semibold text-white hover:text-gray-200 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    href="{{ route('password.request') }}">
                    {{ __('Forgot your password?') }}
                </a>
            @endif

            <x-primary-button class="ml-3 bg-white bg-opacity-50 hover:bg-opacity-40">
                {{ __('Sign in') }}
            </x-primary-button>
        </div>
    </form>
</x-guest-layout>

<style>
    .box-none {
        background: transparent;
        border: none;
    }

    .under-line {
        border-radius: 0px;
        border-bottom: solid 2px white;
    }
</style>
