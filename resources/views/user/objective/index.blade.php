<x-master title="Objective">
    ...
    <section class="relative -top-40 z-20 mx-20 opacity-95">
                    ...
                    <div class="p-8" x-data="{ open: 1 }">
                        ...
                        @foreach ($team as $teams)
                            <div x-show="open === {{ $teams->id }}">
                                ...
                                <form action="{{ route('team.rule.update', ['id' => $teams->id]) }}" method="post">
                                    @method('PUT')
                                    @csrf
                                    ...
                                    <textarea name="team_rule" rows="4"
                                        class="focus:ring-blue-400 focus:border-blue-400 block w-full sm:text-sm border border-black p-2 mb-5"
                                        style="background-color:#FFE3C2" placeholder="チームルールを入力してください">{{ $teams->team_rule }}</textarea>
                                    <div class="flex justify-center items-center">
                                        <x-tryce.parts.button>チームルール編集</x-tryce.parts.button>
                                    </div>
                                </form>
                            </div>
                        @endforeach
        ...
        <div class="flex justify-center">
            <x-tryce.parts.button-a :href="route('objective.show', ['id' => $user->id])" theme="secondary">戻る</x-tryce.parts.button-a>
        </div>
    </section>
    <style>
        .bg-img-module {
